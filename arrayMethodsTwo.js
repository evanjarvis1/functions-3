...
newArr = numbers.map((elem) => elem / 2)
...
let newCities = cities.filter((city) => city.charAt(city.length-1) === `i`)
...
fruits.forEach((elem, index) => {
  if (elem.color === `red`){
    ...
    console.log(`The fruit with ${index} is neither an apple or orange`)
  }
})
...
let foodsArr = foods.filter((food) => food.tags.includes("rice"))

let totalPrice = foodsArr.reduce((acc, cur) => acc + cur.price, 0)
